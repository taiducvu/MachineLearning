...
rng = np.random.RandomState(1150)
...
class Momentum(object):
    ...
    def Update(self, ls_layers):
        ...
        ## Compute gradient descent the cost function
        self.grad_Ws = T.grad(cost, ls_weights)
        self.grad_Bs = T.grad(cost, ls_biases)
        ...
        for layer, grad_w, grad_b, v in zip(ls_layers, self.grad_Ws, self.grad_Bs, self.velocity):
...
class AdaGrad(object):
    ...
    def Update(self, ls_layers):
        ...
        updates = []        
        for layer in ls_layers:
            self.cache.append(theano.shared(np.zeros((layer.weights.get_value().shape[0],layer.weights.get_value().shape[1])),
                                          allow_downcast=True))
            
        for layer, grad_w, grad_b, r in zip(ls_layers, self.grad_Ws , self.grad_Bs, self.cache):
